...
from scan_n100 import parse_cif, Grid, min_dist, HEXXH, READHEAD, LOCK_R253, LOCK_Y217, LOCK_M255, ANCHORS, E123, CUT
import math

BASE_G = "/home/u22607007/ppri_evo_boltz_n100/out_n100_offline/boltz_results_*/predictions/*"
...
for d in dirs:
    ...
    for cif in cifs:
            ...
            for ax, ay, az in atoms:
                for nt, bx, by, bz in grid.near(ax, ay, az):
                    if (ax-bx)**2+(ay-by)**2+(az-bz)**2 <= CUT*CUT:
                        iface_res.add(resi); covered.add(nt); break
        ...
        rows.append(dict(
            ...
            d_e123=round(sany(E123), 2), iface=len(iface_res), ntcov=len(covered)))
