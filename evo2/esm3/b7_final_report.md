...
## §9 PPO 超参扫描 — 2026-08-31

**配置网格**：lr ∈ {1e-4, 3e-4, 1e-3} × ent ∈ {0.01, 0.05}，rollout=1024，2 seeds × 3000 evals。

| 景观 | lr=1e-4 | lr=3e-4 | lr=1e-3 |
|---|---|---|---|
| ppri_additive（数学） | 0.305 | **0.322** | 0.304 |
| surrogate v1（DL 代理） | 0.689 | 0.689 | **0.730** |

- **BEST: lr=1e-3, ent=0.01**（两景观平均 norm 0.517）
- entropy 正则（0.01 vs 0.05）在该预算下**不敏感**（完全相同的 norm）
- lr 是主导超参：代理景观高 lr（1e-3）比默认 3e-4 提升 **+6%**（0.689→0.730）；数学景观 lr=3e-4 最优
- **建议**：代理景观跑 PPO 用 `lr=1e-3, ent=0.01`；数学景观保持默认 3e-4
