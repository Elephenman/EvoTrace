...
## §10 Boltz-2 候选验证（作业 225660，CHPC 4090）— 2026-08-31

**14 yaml（7 候选 × S1 靶/OFF 非靶）× diffusion 20 samples，判读铁律：HEXXH 接触（激活）+ K67/R232 双锁 + S1−OFF 判别。**

| 候选 | act_S1 | act_OFF | sep(判别) | lock_S1 | 结论 |
|---|---|---|---|---|---|
| **K216R** | 0.950 | 0.500 | **0.450** | 0.100 | ✅ 判别 WT×3，新候选最佳 |
| **Q120V** | 0.950 | 0.500 | **0.450** | 0.150 | ✅ 判别 WT×3，新候选最佳 |
| F88Y_Y170F | 0.750 | 0.400 | 0.350 | **0.350** | ✅ 判别 WT×2.3，双锁最高，实验 Top1 保持 |
| K216R_Y170F_Q120V | 0.900 | 0.700 | 0.200 | 0.100 | ⚠️ 组合判别劣于单点（Y170F 拉高 OFF） |
| Y170F | 0.900 | 0.950 | **−0.050** | 0.200 | 非特异激活增强（与机制一致），无判别 |
| WT | 0.500 | 0.350 | 0.150 | 0.050 | 基线 |
| R85A_R207A_R267A | 0.500 | 0.350 | 0.150 | 0.050 | ❌ 负对照与 WT 一致（Boltz 局限，与实验矛盾） |

**关键结论**：
1. **Y170F 初步判读（9/14 时 sep=0.900）被全量数据修正为 −0.050**——OFF 补齐后 Y170 单点对非靶也高激活（90%/95%），无判别。这与"Y170 双锁位点增强非特异激活"的实验机制一致；特异性判别需要 F88 的 dG10 识别协同。
2. **K216R / Q120V 判别 0.45（WT 的 3 倍）**——滑道正电/口袋重塑显著增强"靶 vs 非靶"区分，建议进入下一轮验证。
3. **F88Y_Y170F 双锁比例 0.35 全场最高** + 判别 0.35——实验 Top1 保持成立。
4. **组合 K216R+Y170F+Q120V 判别退化（0.20）**——Y170F 在组合中拉高 OFF，下一轮组合应改为 K216R+Q120V+F88Y_Y170F。
5. **负对照 R85A_R207A_R267A 与 WT 完全一致**——Boltz 状态分布标签无法体现锚点破坏（实验已证 2mM Mn²⁺ 下丧失结合），**Boltz 此指标不可单独用于锚点评估**（诚实披露）。

**下一轮建议**：`K216R + Q120V + F88Y_Y170F` 组合（避开 Y170F 单点）→ Boltz 验证；K216R/Q120V 单点进入湿实验候选。

---

## §11 Boltz-2 第二轮组合验证（作业 225666）— 2026-08-31

**4 yaml（2 组合 × S1/OFF）× 20 samples。基于第一轮结论的组合设计。**

| 组合 | act_S1 | act_OFF | sep(判别) | lock_S1 | 结论 |
|---|---|---|---|---|---|
| **K216R_Q120V** | 0.800 | 0.400 | **0.400** | 0.050 | ✅ 最优组合：判别 WT×2.7，激活 80%，双最优单点叠加不退化 |
| K216R_Q120V_F88Y_Y170F | 0.800 | 0.750 | 0.050 | 0.100 | ⚠️ 双锁加入拉高 OFF（0.75），判别丢失 |

**机制结论（两轮 Boltz 验证汇总）**：
1. **双锁位点（F88/Y170）= 非特异激活增强器**（S1/OFF 无差别拉高）——与项目机制"双锁改造极大增强非特异性 ssDNA 激活"完全自洽；F88Y_Y170F 单独用时双锁比例 0.35 最高。
2. **K216R/Q120V = 特异性判别器**（滑道正电 + 口袋重塑，判别 WT×3）。
3. **最优平衡 = K216R+Q120V**（判别 0.40 + 激活 80%）；**含 Y170F 的组合会牺牲判别**。
4. **GateF 方向的实验启示**：要让"双锁激活增强"与"靶标特异性"共存，需要把双锁做成激活**必要条件**（未双锁的 DNA 无法激活）——这正是 GateF 概念；当前 Boltz 数据显示双锁在复合物模型中无差别增强，印证了"双锁本身不提供判别，判别必须由碱基识别层（K216R/Q120V + F88 的 dG10 偏好）提供"。

**最终候选矩阵（湿实验候选）**：
- 单点：K216R / Q120V（判别 0.45）
- 组合：K216R+Q120V（判别 0.40，最优平衡）
- 保持：F88Y_Y170F（双锁 0.35，激活增强路径）

---

## §12 工具完结版（v1.0）— 最终全量对比（2026-08-31）

**集成**：ESM3 v3 代理正式接入 b7 主线（`surrogate_ppri_v3` 工厂，DeepSetV3 + PCA + ESM3 emb）；PPO 最优超参回填（lr=1e-3, ent=0.01）。3 景观 × 六内核 × 5 seeds × 4000 evals。

| 景观 | 🥇 | 2nd | 3rd | 4th | 5th | 6th |
|---|---|---|---|---|---|---|
| **surrogate_ppri_v3 (ESM3 代理)** | WF **0.9999** | PPO **0.9999** | DQN 0.9534 | CEM 0.3755 | BC 0.1955 | ES −0.0477 |
| **surrogate_ppri (v1 代理)** | WF **0.8975** | PPO 0.7508 | DQN 0.5994 | CEM −0.1734 | BC −0.4539 | ES −2.1671 |
| **ppri_additive (数学)** | WF **0.9155** | CEM 0.5743 | ES 0.3529 | PPO 0.2935 | DQN 0.2341 | BC 0.0433 |

**工具 v1.0 定版结论**：
1. **WF（先验提议）三景观全冠**——跨 DL 代理/数学景观唯一稳定最优，工具默认内核。
2. **PPO 是唯一在 ESM3 代理景观达到 ref 的 RL 内核**（0.9999 并列 WF）——lr=1e-3 回填后 v1 代理 0.719→0.751（+4%）。
3. **v3 代理景观 ref=0.9165 恰为 216 饱和位点**——WF/PPO 均 exploit 到该点，验证工具能定位代理"伪最优"，同时提示 ESM3 代理在 PprI 零样本的绝对尺度偏置（见 §8.3）。
4. **分布类方法（CEM/ES）在代理景观塌缩、数学景观可用的景观×方法强交互**再次确认。
5. 完结交付：b7 集成（v3 工厂 + PPO 超参）、三景观全量结果、本节定版结论。

**工具 v1.0 完结**。已知边界（诚实披露）：ESM3 代理零样本偏置（§8.3）；Boltz-2 无法区分锚点破坏（§10）；代理排序≠生理学。

---

## §13 代理校准层 v1（Boltz 标签 → ESM3 代理）— 2026-08-31

**目标**：工具"代理快筛 + 昂贵标签回流"闭环。用 Boltz-2 状态标签校准 ESM3 代理的零样本偏置（§8.3）。

**数据集**：40 系统（reference 8 + wave0 24 精英 + 本轮 v3cand 8），标签 = HEXXH 激活率（S1 靶）。

**相关性（代理 fitness vs Boltz 标签）**：

| 对比 | Spearman | n | 解读 |
|---|---|---|---|
| proxy vs **act_s1** | **0.605** | 40 | 代理携带激活信号，可校准 |
| proxy vs act_off | 0.476 | 38 | 非靶激活弱相关 |
| proxy vs **sep（判别）** | **0.040** | 38 | **代理无 DNA 维度，无法预测判别**——判别必须走 Boltz 层 |

**Isotonic 校准（LOO CV，代理 z-score → 激活概率）**：
- MSE 0.0386 → **0.0282（−27%）**；校准后 Spearman 0.421。
- 资产：`calib_isotonic.npz`（阈值表），用法：`P(激活|S1) = isotonic(proxy_fit)`。

**关键发现（诚实披露）**：
1. **代理对多突变组合系统性低估**——wave0 精英（12 突变）proxy_fit 全 −0.7~−1.0，但 Boltz 激活 35-70%；单/双突变（v3cand）代理准（K216R 0.916 ↔ act 0.95）。→ 代理适合**少突变快筛**，多突变必须 Boltz 层验证。
2. **判别（S1−OFF）不可由序列代理预测**（rho=0.04）——特异性评估是 Boltz/实验的专属能力。
3. 校准改善 27% 但绝对值仍 ~0.17 激活概率误差——代理是"粗筛"，不是"测量"。

**工具闭环**：代理快筛（少突变，rho 0.605）→ 校准概率门槛 → 精英进 Boltz-2 验证（判别/双锁）→ 标签回流扩充校准集。**v1.1 能力**。
